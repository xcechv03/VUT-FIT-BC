...
*/
    ...
    return q->b_index == q->f_index ? 1 : FALSE;
}
...
*/
    ...
    return nextIndex(q->b_index) == q->f_index ? 1 : FALSE;
}
...
*/
    ...
    if(queueEmpty(q) == 0)
    {
        *c = q->arr[q->f_index]; //vlozime znak
    }
    else  queueError(QERR_FRONT); //fronta je prazdna
...
*/
    ...
    if(queueEmpty(q)){          //fronta je prazdna
        queueError(QERR_REMOVE);
        return;
    }
    q->f_index = nextIndex (q->f_index);          //fronta nie je prazdna
...
*/
    ...
    {
        q->arr[q->b_index] = c;
        q->b_index = nextIndex(q->b_index);         //vlozime a posunieme sa
    }
    else queueError(QERR_UP);  //fronta je plna
