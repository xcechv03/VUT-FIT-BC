...
**/
    ...
    if(*RootPtr == NULL){
        ...
        tBTNodePtr temporary = (*RootPtr);
        do{
            if(Content < temporary->Cont){
                if(temporary->LPtr)
                    temporary = temporary->LPtr;
                else{
            ...
            else if(Content > temporary->Cont){
                if(temporary->RPtr)
                    temporary = temporary->RPtr;
                else{
                    tBTNodePtr novy = malloc(sizeof(struct tBTNode));
                    novy->Cont = Content;
                    novy->RPtr = NULL;
...
**/
    ...
    tBTNodePtr temporary = RootPtr;
    while(!SEmptyP(&Stack)){
        temporary = STopPopP(&Stack);
        Leftmost_Preorder(temporary->RPtr,&Stack);
    }
}
...
**/
    ...
    while(!SEmptyP(&Stack)){
        temporary = STopPopP(&Stack);
        BTWorkOut(temporary);
        Leftmost_Inorder(temporary->RPtr,&Stack);
    }
}
